...
- **No frontend.** This repository is the REST API only. The prototype at
  ...
  here renders it.
- **No real SMS vendor.** `SmsService` supports a `log` driver (writes
  `sms_log` and sends nothing — the seeded default) and a generic `http`
  driver (URL + bearer token + sender name, encrypted the same way the SMTP
  password is) with no vendor-specific adapter behind it. `admin/sms/settings`
  lets an admin configure the generic driver, but nothing in this codebase
  has been exercised against a real provider.
- **No S3 (or any object-storage) backend.** Every uploaded file — public
  media assets and applicants' private scholarship documents alike — is
  written to local disk under `STORAGE_ROOT`. There is no storage-driver
  abstraction to swap in a bucket later; `STORAGE_ROOT` is read directly by
  `MediaService`/`PrivateFileStore`.
- **No automated backups beyond a documented `mysqldump` cron.**
  `DEPLOYMENT-HOSTINGER.md` describes a cron-scheduled `mysqldump` plus a
  rotation policy as the whole backup story. There is no restore tooling, no
  point-in-time recovery, and nothing backs up `STORAGE_ROOT` itself (the
  uploaded files) — only the database.
...
- **`docker-compose.yml`'s MySQL 8 image is a development convenience, not
  what production runs on.** `DEPLOYMENT-HOSTINGER.md` targets MariaDB
